    ...
    controller.on('slash_command', function(bot, message) {
        ...
        switch(message.command) {
			case '/lookup': // change this to your command
				if(message.text === '') return bot.replyPrivate(message, 'Please enter a domain');
				return SlashCommands.lookup(controller.storage, bot, message);
			case '/chat': // change this to your command
				if(message.text === '') return bot.replyPrivate(message, 'Please enter a message');
				return SlashCommands.chat(controller.storage, bot, message);
			default:
				bot.replyPrivate(message, `You just typed "${message.text}" using command "${message.command}"`);
        }
    });
}
